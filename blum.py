...
class BlumBot:
    def __init__(self, thread: int, session_name: str, phone_number: str, proxy: [str, None]):
        ...
        # Initialize counters for bombs and snowflakes
        self.snowflakes_used = 0
        self.bombs_used = 0
        self.snowflake_game_counter = 0
        self.bomb_game_counter = 0
    ...
    async def play_game(self):
        ...
        while play_passes:
            ...
            if not isinstance(msg, bool) and msg:
    ...
    async def claim_game(self, game_id: str):
        base_points = random.randint(*config.POINTS)
        snowflakes = random.randint(0, config.SNOWFLAKES[2]) if self.snowflake_game_counter < config.SNOWFLAKES[
            1] else 0
        bombs = random.randint(0, config.BOMBS[2]) if self.bomb_game_counter < config.BOMBS[1] else 0
        self.snowflake_game_counter += 1
        self.bomb_game_counter += 1

        snowflake_bonus = snowflakes * random.uniform(0.05, 0.17)
        bomb_penalty = bombs * random.uniform(0.2, 0.8)
        await self.session.post("https://game-domain.blum.codes/api/v1/game/un/bombs", json={"gameId": game_id, "bombs": bombs},
                                proxy=self.proxy)
        await self.session.post("https://game-domain.blum.codes/api/v1/game/un/snowflakes", json={"gameId": game_id, "snowflakes": snowflakes},
                                proxy=self.proxy)
        points = int(base_points * (1 + snowflake_bonus) * (1 - bomb_penalty))

        max_points = config.POINTS[1]
        ...
        logger.success(
            f"Thread {self.thread} | {self.account} | Finish play in game! Bombs: {bombs}, Snowflakes: {snowflakes}; Reward: {points}")
